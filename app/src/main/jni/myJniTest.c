...
static void dealWith_bitmap( AndroidBitmapInfo*  info, void*  pixels){
...
*/
        uint16_t*  line_end = line + (info->width)*2;

        if (line < line_end) {

//            line[0] = make565(0,255,0);
//            line++;
...
JNIEXPORT jint JNICALL Java_luofeng_myjnitest_jni_JniHelper_renderPlasma(JNIEnv * env
        , jobject  obj, jobject bitmap,  jlong  time_ms) {
    //bitmap信息
    ...
    //获取到bitmap的信息，也就是info的值
    if ((ret = AndroidBitmap_getInfo(env, bitmap, &info)) < 0) {
        LOGE("AndroidBitmap_getInfo() failed ! error=%d", ret);
        return;
    }

    //这里处理bitmap默认操作的颜色模式是RGB_565:一个像素由两个字节表示，由高到低RGB分别由5位，6位，5位表示的，模式不对则退出操作
    if (info.format != ANDROID_BITMAP_FORMAT_RGB_565) {
        LOGE("Bitmap format is not RGB_565 !");
        return;
    }

    //锁定像素点在内存中的位置，便于操作，并且传入一个空的引用获取到像素点的位置
